...
- 产品原型：已确认紧凑型多股票卡片、卡片内消息通知、点击放大详情
...
7. 每只股票卡片底部用独立消息框展示该股票的买卖通知，页面顶部不再展示集中通知栏；
...
启用的监控股票必须通过仅限回环地址访问的监控清单接口提供给 Longbridge
WebSocket 订阅器；禁用股票不得进入该强制订阅集合。严格 1 分钟查询必须优先使用
`lb_realtime_candlesticks` 的 `min_1` WebSocket K 线，并以
`lb_minute_bars`、`lb_intraday_lines` 补齐历史或缺失分钟；同一股票和分钟出现多源
记录时，WebSocket K 线优先。

卡片行情必须显示权威实时快照的价格和涨跌幅。WebStock 在同一分钟的增量快照暂时
缺少昨收价或涨跌幅时，查询层必须使用同一股票、同一交易日内最近一条非空昨收价
补齐，并以最新价和昨收价重新计算涨跌额与涨跌幅。不能因为最后一条增量记录字段为空
而隐藏涨跌幅，也不得沿用与最新价不一致的旧涨跌幅；不得跨交易日沿用基准。
...
页面顶部不得重复展示股票买卖通知；通知必须归入对应股票卡片，避免占用
K 线总览上方空间。
...
价格方向颜色规范（2026-07-24 批准）：卡片价格与涨跌幅必须使用相同的市场方向颜色。
上涨使用现有的 `text-bull` 红色，下跌使用现有的 `text-bear` 绿色，平盘使用
`text-muted` 中性色。本规范不改变道氏买卖信号的颜色语义。

卡片消息通知规范（2026-07-24 批准）：迷你 K 线下方必须设置 128 像素高且可滚动
的消息文本框，按通知接口返回顺序展示该股票的通知。通知必须直接使用纯文本行展示，
不得为每条通知嵌套圆角卡片、独立背景或四周边框；相邻通知只允许使用轻量分隔线。
每只股票返回的第一条通知视为该股票的最新消息，必须显示“最新”文字标记并使用左侧
强调线突出；该标记不得重新引入独立背景、圆角或四周边框。
每条消息必须同时显示中文操作、
信号触发周期、信号触发时间、信号触发价格和中文形态，并保留未读通知的“已读”
操作。周期、时间与价格必须直接取自对应不可变通知的 `timeframe`、`triggered_at`
和 `trigger_price`，不得使用当前选中的卡片周期、行情刷新时间或监控成功时间代替。
消息框没有通知时显示“暂无消息通知”，通知加载中或失败时在框内显示对应状态。
...
5. 股票消息通知框固定在卡片底部，不得覆盖或缩小迷你 K 线；
6. 不改变趋势线、买卖点、五周期状态或数据新鲜度的计算与语义。

时间显示规范（2026-07-24 批准）：监控页面展示的行情时间、成功时间、数据源时间、
信号触发时间和详情更新时间必须统一转换为北京时间（`Asia/Shanghai`，UTC+8），
不得直接显示 UTC `Z` 时间。后端存储和不可变通知快照继续保留原始时区语义，转换
只发生在展示层。
...
1. 宽屏四列紧凑卡片；
2. 页面顶部不显示集中通知栏，每张卡片底部消息框显示该股票通知；
3. `ALL / A股 / 港股 / 美股` 同时过滤卡片及其消息；
4. 市场筛选不修改监控开关；
5. 买点、卖点、观察、数据延迟颜色语义；
6. 点击卡片打开完整 K 线和指标；
7. 关闭详情后保留筛选及滚动位置；
